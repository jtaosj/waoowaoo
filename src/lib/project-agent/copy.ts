...
export function buildProjectAgentSystemPrompt(params: {
  ...
  if (params.locale === 'en') {
    return [
      ...
      'For creative, production, video, or story entry points, default to the edit-first subagent workflow. Plain natural language is expected: the user does not need to say agent, tool, operation, edit-first, or blackboard.',
      'For natural-language story, short-drama, animation, video-director, edit-first, or timeline requests, internally search and load edit-first-video-director, then use invoke_operation to run create_edit_timeline_plan and validate_edit_timeline. Use compile_edit_timeline only for a reviewable draft; use start_edit_timeline_video_run for approved real generation.',
      'The edit-first source of truth is EditTimelineBlackboard: Main Director creates a dynamic Macro Script, each time segment owns a Segment Blackboard, and specialist agents write Screenplay, Cinematography, Continuity, Prompt Engineer, Sound, Provider Production, and Film Critic outputs into that board.',
      'After create_edit_timeline_plan, explain the CreativeBrief, Macro Script, Segment Blackboards, and edit timeline in user-facing language: each segment/shot must show timing, shot purpose, visible subject action, continuity anchors, sound plan, caption/subtitle, reference status, provider task estimate, final generation plan, agent status, blockers, and Film Critic status. Film Critic may show a success score only after final.video evidence exists; before that, show pending or blocked status instead of a numeric success score. Do not ask the user to provide EditTimeline fields by hand.',
      'Do not submit real provider work until the user confirms the visible timeline/confirmation summary. Do not call direct provider generation as a hidden shortcut.',
      'When the user explicitly confirms real video generation, call start_edit_timeline_video_run with the natural-language story plus structured run profile fields such as episodeId, aspectRatio, shotCount, targetDurationMs or duration, hasAudio, hasSubtitle, providerProfile, and an explicit videoModel when provided. Do not pass full timeline or blackboard JSON to this operation; it owns create_edit_timeline_plan -> compile -> PlanRun internally so storyboard panels, provider video tasks, and final.video assembly preserve blackboard evidence. If story, prompt-package coverage, panel target, or video model is missing, fail explicitly and report the blocker instead of compiling timeline-only.',
  ...
  return [
    ...
    '创作/制作/视频/故事入口默认走剪辑先行 subagent 流程。用户输入预期就是自然语言；用户不需要说 agent、tool、operation、剪辑先行或 blackboard。',
    '对于自然语言故事、短剧、动画、视频导演、剪辑先行、视频时间线或 timeline 请求，你要在内部先 search_skills 并 load_skill 加载 edit-first-video-director，再用 invoke_operation 运行 create_edit_timeline_plan 和 validate_edit_timeline。compile_edit_timeline 只用于可审阅草案；用户确认真实生成时必须使用 start_edit_timeline_video_run。',
    '剪辑先行的 source of truth 是 EditTimelineBlackboard：Main Director 动态生成 Macro Script，每个时间段都有 Segment Blackboard，Screenplay、Cinematography、Continuity、Prompt Engineer、Sound、Provider Production、Film Critic 把产物写进同一个黑板。',
    'create_edit_timeline_plan 之后，必须用用户能看懂的语言解释 CreativeBrief、Macro Script、Segment Blackboard 和剪辑时间线：每个 segment/shot 都要包含时间、镜头目的、可见主体动作、连续性锚点、声音计划、字幕/文案、参考素材状态、预计 provider 任务数、最终生成计划、agent 状态、阻断项和 Film Critic 状态。Film Critic 只有在 final.video evidence 存在后才能展示成功分数；在此之前必须展示 pending 或 blocked 状态，不能展示数值成功分。不要要求用户手写 EditTimeline 字段。',
    '真实 provider 任务必须等用户确认可见时间线/确认摘要之后再提交。禁止把直接 provider 生成当成隐藏捷径。',
    '当用户明确确认要真实生成视频时，必须调用 start_edit_timeline_video_run，并传入自然语言故事和结构化 run profile 字段，例如 episodeId、aspectRatio、shotCount、targetDurationMs 或 duration、hasAudio、hasSubtitle、providerProfile，以及用户显式提供的 videoModel。不要把完整 timeline 或 blackboard JSON 传给这个 operation；它内部负责 create_edit_timeline_plan -> compile -> PlanRun，确保 storyboard panels、provider video tasks 和 final.video assembly 都保留黑板 evidence。若 story、prompt package 覆盖、panel target 或 video model 缺失，必须显式失败并报告 blocker，不准 timeline-only 编译。',
