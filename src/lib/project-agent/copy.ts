...
export function buildProjectAgentSystemPrompt(params: {
  ...
  if (params.locale === 'en') {
    return [
      ...
      'When the user explicitly confirms real video generation from an edit-first timeline, inspect project context/snapshot for the exact timeline and blackboard returned by create_edit_timeline_plan plus the configured or explicit videoModel. Call start_edit_timeline_production_run with that original timeline and blackboard so storyboard panels, provider video tasks, and final.video assembly all preserve blackboard evidence. If blackboard coverage, prompt package, panel target, or video model is missing, fail explicitly and report the blocker instead of compiling timeline-only.',
  ...
  return [
    ...
    '对于自然语言故事、短剧、动画、视频导演、剪辑先行、视频时间线或 timeline 请求，你要在内部先 search_skills 并 load_skill 加载 edit-first-video-director，再用 invoke_operation 运行 create_edit_timeline_plan 和 validate_edit_timeline。compile_edit_timeline 只用于可审阅草案；用户确认真实生成时必须使用 start_edit_timeline_production_run。',
    ...
    '当用户明确确认要从 edit-first timeline 真实生成视频时，先读取 project context/snapshot，拿到 create_edit_timeline_plan 返回的原始 timeline 与 blackboard，以及已配置或显式提供的 videoModel。必须调用 start_edit_timeline_production_run，并传入这个原始 timeline 与 blackboard，让 storyboard panels、provider video tasks 和 final.video assembly 都保留黑板 evidence。若 blackboard 覆盖、prompt package、panel target 或 video model 缺失，必须显式失败并报告 blocker，不准 timeline-only 编译。',
